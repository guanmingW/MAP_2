...
void main()
{
    ...
    vec2 vUv=fract(6.0*uv);                 //key, fract取小數
    vec4 shadeColor= texture2D(u_tex0, uv); //取圖
    float shading= shadeColor.g;            //g=green, 取圖綠色版作為明亮值(0~1)
    ...
    vec4 c;
                ...
                if( shading > 5. * step )
                {
                    c = mix( texture2D( u_tex1, vUv ), vec4( 1. ), 6. * ( shading - 5. * step ) );//vec4(1.)==vec4(1., 1., 1., 1.,)也就是白色
                }
